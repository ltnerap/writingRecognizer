...
My code will recognize all 26 letters of the alphabet (with some difficulty on B and G, but generally correct for other letters). There is a perceptron that is trained for each letter, and when the user runs ocr test, I get the prediction from every perceptron and feed back the letter for whichever perceptron predicts 1.
...
//TODO: MAKE SOME NEURAL NETS AND TRAIN THEM HERE, THEN SAVE THE WEIGHTS TO perceptron.txt
	//creates perceptron with enough nodes for every pixel on the screen. deleted at end of function
	int numNets = 0;
	for(int i=0; i<linecount; i++){
		//create perceptron for first sample
		if(i == 0){
			nets[numNets] = new Perceptron(GRIDWIDTH*GRIDHEIGHT);
			//put letter into array of non-repeating letters
			letters[0] = sample_output[0];
			numNets++;
		}
		else{
			//if there isn't already a perceptron for this letter, make a new perceptron and add the letter to the array of non-repeating letters
			if(sample_output[i] != sample_output[i-1]){
				nets[numNets] = new Perceptron(GRIDWIDTH*GRIDHEIGHT);
				letters[numNets] = sample_output[i];
				numNets++;
			}
		}
	}
	//counts number of times perceptron has answered all correctly
	
	for(int j=0; j<numNets; j++){
		int isTrained = 0;
		//while perceptron hasn't answered all correctly 100 times
		while(isTrained < 100){
			//times correct out of linecount
			int trainCount = 0;
			
			for(int i=0; i<linecount; i++){
				bool trainingOutput = false;
				//if letter is correct letter, train net with 1
				if(sample_output[i] == letters[j]){
					trainingOutput = nets[j]->train(sample_input[i], 1);
				}
				//else, train net with 0
				else{
					trainingOutput = nets[j]->train(sample_input[i], 0);
				}
					//add to number of times answered correctly if train returns true
				if(trainingOutput){
					trainCount++;
				}
			
			}
			//if it answered all correctly, add to number of perfect iterations
			if(trainCount == linecount){
				isTrained++;
			}
		}
	}

	//print predictions for each line
	for(int j=0; j<numNets; j++){
		for(int i=0; i<linecount; i++){
			cout << "The " << letters[j] << " perceptron's prediction for " << sample_output[i] << ": " << nets[j]->getPrediction(sample_input[i]) << endl;
		}
	}

	//put all weights into perceptron.txt
	ofstream perceptronFile;
	perceptronFile.open("perceptron.txt",ios::out|ios::app);
	//number of nets is first thing in perceptron.txt
	perceptronFile << numNets << endl;

	for(int k=0; k<numNets; k++){
		perceptronFile << letters[k] << endl;
		//put in output weights
		for(int i=0; i<nets[k]->size + 1; i++){
			perceptronFile << nets[k]->outputweight[i] << endl;
		}
		//put in hidden weights
		for(int i=0; i<nets[k]->size; i++){
			for(int j=0; j<nets[k]->size + 1; j++){
			perceptronFile << nets[k]->hiddenweight[i][j] << endl;
			}
		}
	}
	//close perceptron.txt and delete the nets
...
//TODO: MAKE SOME NEURAL NETS, READ THE WEIGHTS FROM A FILE perceptron.txt, USE THE NEURAL NETS TO IDENTIFY THE LETTER
	//make a new net to add weights to
	char letters[26];
	Perceptron* nets[26];
	//read weights from perceptron.txt and put them into weight arrays for perceptron
	ifstream weightFile;
	string line;
	weightFile.open("perceptron.txt");
	if(!weightFile.is_open()){
		cout << "Couldn't open perceptron.txt" << endl;
		return;
	}

	//read number of nets needed from file
	int numNets = 0;
	weightFile >> numNets;
	for(int k=0; k<numNets; k++){
		weightFile >> letters[k];
		nets[k] = new Perceptron(GRIDWIDTH*GRIDHEIGHT);
		//put in output weights
		for(int i=0; i<nets[k]->size + 1; i++){
			weightFile >> nets[k]->outputweight[i];
		}
		//put in hidden weights
		for(int i=0; i<nets[k]->size; i++){
			for(int j=0; j<nets[k]->size + 1; j++){
				weightFile >> nets[k]->hiddenweight[i][j];
			}
		}
	}

	//print out letter of whichever perceptron returns 1
	for(int i=0; i<numNets; i++){
		if(nets[i]->getPrediction(getSquares()) == 1){
			cout << "The letter is " << letters[i] << endl;
		}
	}
}
